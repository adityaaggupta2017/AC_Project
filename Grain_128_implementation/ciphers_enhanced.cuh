...
// ============================================================
// ============================================================
// AES-128 - Advanced Encryption Standard (GPU Optimized)
// Block size: 128 bits, Key size: 128 bits
// Rounds: 10
// Optimized for GPU-accelerated brute-force attacks on partial keys
// ============================================================

static constexpr int AES128_ROUNDS = 10;
static constexpr int AES128_KEY_BYTES = 16;
static constexpr int AES128_BLOCK_BYTES = 16;

// Helper function to get S-box value (constexpr version for CUDA __host__ __device__)
__host__ __device__ static inline uint8_t aes_sbox(uint8_t idx) {
...
// Galois field multiplication by 2 (optimized fast path)
__host__ __device__ static inline uint8_t gmul2(uint8_t x) {
  return (uint8_t)((x << 1) ^ (((x >> 7) & 1) * 0x1b));
}

// Galois field multiplication by 3 (uses gmul2 for efficiency)
__host__ __device__ static inline uint8_t gmul3(uint8_t x) {
  return (uint8_t)(gmul2(x) ^ x);
}

// General Galois field multiplication (naive path for comparison)
__host__ __device__ static inline uint8_t gmul(uint8_t a, uint8_t b) {
  uint8_t p = 0;
  #pragma unroll
  for (int i = 0; i < 8; i++) {
    if ((b & 1) != 0) p ^= a;
    uint8_t hi_bit_set = (uint8_t)((a & 0x80) != 0 ? 1 : 0);
    a = (uint8_t)(a << 1);
...
__host__ __device__ static inline uint32_t rotword(uint32_t word) {
  return ((word << 8) | (word >> 24));
}

// Apply S-box to each byte in a word
__host__ __device__ static inline uint32_t subword(uint32_t word) {
  return (((uint32_t)aes_sbox((word >> 24) & 0xFF)) << 24) |
         (((uint32_t)aes_sbox((word >> 16) & 0xFF)) << 16) |
         (((uint32_t)aes_sbox((word >> 8) & 0xFF)) << 8) |
         ((uint32_t)aes_sbox(word & 0xFF));
}

// Round constant generator
__host__ __device__ static inline uint8_t rcon(int round) {
  constexpr uint8_t rcon_vals[10] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};
  return (round < 10) ? rcon_vals[round] : 0x00;
}

__device__ __constant__ uint32_t AES_T0[256] = {
0xa56363c6, 0x847c7cf8, 0x997777ee, 0x8d7b7bf6, 0x0df2f2ff, 0xbd6b6bd6, 0xb16f6fde, 0x54c5c591,
0x50303060, 0x03010102, 0xa96767ce, 0x7d2b2b56, 0x19fefee7, 0x62d7d7b5, 0xe6abab4d, 0x9a7676ec,
0x45caca8f, 0x9d82821f, 0x40c9c989, 0x877d7dfa, 0x15fafaef, 0xeb5959b2, 0xc947478e, 0x0bf0f0fb,
0xecadad41, 0x67d4d4b3, 0xfda2a25f, 0xeaafaf45, 0xbf9c9c23, 0xf7a4a453, 0x967272e4, 0x5bc0c09b,
...
struct AES128 {
  uint32_t round_keys[44];  // 11 round keys × 4 words each

  // Fast key expansion with vectorized operations for GPU
  __host__ __device__ static inline void expand_key(const uint8_t key[16], uint32_t out_rk[44]) {
    #pragma unroll
    for (int i = 0; i < 4; i++) {
      out_rk[i] = ((uint32_t)key[4*i] << 24) |
    ...
    for (int i = 4; i < 44; i++) {
      ...
      if ((i % 4) == 0) {
        temp = subword(rotword(temp)) ^ (((uint32_t)rcon((i / 4) - 1)) << 24);
      }
      out_rk[i] = out_rk[i-4] ^ temp;
    }
  }

  // SubBytes transformation with S-box lookup
  __host__ __device__ static inline void subbytes(uint8_t state[16]) {
    #pragma unroll
    for (int i = 0; i < 16; i++) {
  ...
  __host__ __device__ static inline void shiftrows(uint8_t state[16]) {
    uint8_t temp;
  ...
  // MixColumns transformation with optimized GF arithmetic (fast path)
  __host__ __device__ static inline void mixcolumns_fast(uint8_t state[16]) {
    #pragma unroll
    for (int i = 0; i < 4; i++) {
      ...
      state[4*i]   = (uint8_t)(gmul2(s0) ^ gmul3(s1) ^ s2 ^ s3);
      state[4*i+1] = (uint8_t)(s0 ^ gmul2(s1) ^ gmul3(s2) ^ s3);
      state[4*i+2] = (uint8_t)(s0 ^ s1 ^ gmul2(s2) ^ gmul3(s3));
      state[4*i+3] = (uint8_t)(gmul3(s0) ^ s1 ^ s2 ^ gmul2(s3));
    }
  }

  // MixColumns transformation with general GF multiplication (naive path)
  __host__ __device__ static inline void mixcolumns_naive(uint8_t state[16]) {
    #pragma unroll
    for (int i = 0; i < 4; i++) {
      uint8_t s0 = state[4*i];
      uint8_t s1 = state[4*i+1];
      ...
      state[4*i]   = (uint8_t)(gmul(0x02, s0) ^ gmul(0x03, s1) ^ s2 ^ s3);
      state[4*i+1] = (uint8_t)(s0 ^ gmul(0x02, s1) ^ gmul(0x03, s2) ^ s3);
      state[4*i+2] = (uint8_t)(s0 ^ s1 ^ gmul(0x02, s2) ^ gmul(0x03, s3));
      state[4*i+3] = (uint8_t)(gmul(0x03, s0) ^ s1 ^ s2 ^ gmul(0x02, s3));
    }
  }

  // AddRoundKey transformation - XOR with round key
  __host__ __device__ static inline void addroundkey(uint8_t state[16], const uint32_t rk[4]) {
    #pragma unroll
    for (int i = 0; i < 4; i++) {
      uint32_t word = rk[i];
      state[4*i]   ^= (uint8_t)((word >> 24) & 0xFF);
      state[4*i+1] ^= (uint8_t)((word >> 16) & 0xFF);
      state[4*i+2] ^= (uint8_t)((word >> 8) & 0xFF);
      state[4*i+3] ^= (uint8_t)(word & 0xFF);
    }
  }

  // Core AES-128 encryption: plaintext → ciphertext (template with optimization control)
  template<bool UseTTable>
  __host__ __device__ static inline void encrypt(const uint8_t plaintext[16], const uint8_t key[16], uint8_t ciphertext[16]) {
    uint32_t rk[44];
    expand_key(key, rk);
    ...
    for (int round = 1; round < AES128_ROUNDS; round++) {
      ...
      if constexpr (UseTTable) {
        mixcolumns_fast(ciphertext);
      } else {
        mixcolumns_naive(ciphertext);
      }
      addroundkey(ciphertext, &rk[4 * round]);
    }

    // Final round (no MixColumns)
    subbytes(ciphertext);
    shiftrows(ciphertext);
    addroundkey(ciphertext, &rk[40]);
  }

  // Optimized matcher for known-plaintext brute-force attacks (template with optimization control)
  // Returns true if encryption matches expected ciphertext (early exit on mismatch)
  template<bool UseTTable>
  __host__ __device__ static inline bool encrypt_match(const uint8_t plaintext[16],
    ...
    for (int round = 1; round < AES128_ROUNDS; round++) {
      ...
      shiftrows(state);
      if constexpr (UseTTable) {
        mixcolumns_fast(state);
      } else {
        mixcolumns_naive(state);
      }
      addroundkey(state, &rk[4 * round]);
    }
    ...
    // Early exit: compare each byte and return false immediately on mismatch
    #pragma unroll
    for (int i = 0; i < 16; i++) {
      if (state[i] != expected_ct[i]) return false;
    }
    return true;
  }

  // Batch encryption interface for GPU kernels (single plaintext, multiple keys, with optimization control)
  template<bool UseTTable>
  __host__ __device__ static inline void encrypt_batch(const uint8_t plaintext[16],
                                                        const uint8_t key[16],
                                                        uint8_t ciphertext[16]) {
    encrypt<UseTTable>(plaintext, key, ciphertext);
  }
};
...
// Based strictly on NIST LWC Finalist Specification: https://csrc.nist.gov/CSRC/media/Projects/lightweight-cryptography/documents/finalist-round/updated-spec-doc/grain-128aead-spec-final.pdf
// ============================================================
struct Grain128AEADv2 {
    ...
    // Clocks the cipher and returns the pre-output bit.
    // Handles the complex feedback logic required during initialization.
    ...
    __host__ __device__ static inline void init(const uint8_t key[16], const uint8_t nonce[12],
        ...
        // 320 clocks
        for (int t = 0; t < 320; t++) {
            clock_cipher(lfsr, nfsr, 0, 0, true);
        }

        // 64 clocks
        for (int t = 0; t < 64; t++) {
            // LFSR gets key bits 64->127. NFSR gets key bits 0->63.
            clock_cipher(lfsr, nfsr, key_bits[t + 64], key_bits[t], true);
        }

        // Init MAC Accumulator (64 clocks)
        for (int j = 0; j < 64; j++) {
            auth_acc[j] = clock_cipher(lfsr, nfsr, 0, 0, false);
        }

        // Init MAC Shift Register (64 clocks)
        for (int j = 0; j < 64; j++) {
            auth_sr[j] = clock_cipher(lfsr, nfsr, 0, 0, false);
        }
    }
    ...
    // Helper to process a single byte of AD (including DER length bytes)
    ...
    __host__ __device__ static inline void process(const uint8_t* pt, uint8_t* ct, int pt_len, 
        ...
        // Process AD Length (Strict DER Encoding Requirement)
        if (ad_len < 128) {
        ...
        for (int i = 0; i < ad_len; i++) {
            process_ad_byte(ad[i], lfsr, nfsr, auth_acc, auth_sr);
        }
        ...
        for (int i = 0; i < 8; i++) {
            uint8_t tag_byte = 0;
            for (int bit = 0; bit < 8; bit++) {
                tag_byte |= (auth_acc[i * 8 + bit] << bit);
            }
            tag[i] = tag_byte;
        }
    }

    // Early Reject Matching
...
struct Grain128AEADv2_Bitsliced {
    // Computes 32 lanes simultaneously
    ...
    __host__ __device__ static inline void init(const uint32_t key_bitsliced[128], const uint32_t nonce_bitsliced[96], uint32_t lfsr[128], uint32_t nfsr[128]) {
        ...
        for (int t = 0; t < 64; t++) {
            clock_cipher(lfsr, nfsr, key_bitsliced[t + 64], key_bitsliced[t], true);
        }
    }

    // Takes 32 keys and returns a bitmask of which ones successfully generated the target keystream
    __host__ __device__ static inline uint32_t match_keys(const uint32_t key_bitsliced[128], const uint32_t nonce_bitsliced[96], int ad_len, const uint8_t* pt, const uint8_t* ct, int pt_len) {
        ...
        // Calculate exactly how many clocks we need to blindly fast-forward
        // Phase 3 & 4 MAC init = 128 clocks
        int der_len = (ad_len < 128) ? 1 : (ad_len < 256) ? 2 : 3;
        int ad_clocks = (der_len + ad_len) * 16; // 8 bits * 2 clocks per bit
        int skip_clocks = 128 + ad_clocks;

        // Fast-forward directly to the ciphertext!
        for (int i = 0; i < skip_clocks; i++) clock_cipher(lfsr, nfsr);

        uint32_t match_mask = 0xFFFFFFFF; // Assume all 32 lanes are correct initially

        for (int i = 0; i < pt_len; i++) {
            for (int bit = 0; bit < 8; bit++) {
                uint32_t z_i = clock_cipher(lfsr, nfsr); // Even bit (Keystream)
                clock_cipher(lfsr, nfsr);                // Odd bit (MAC, ignored)
                
                // Target Keystream bit is Plaintext XOR Ciphertext
                uint32_t ks_bit = ((pt[i] ^ ct[i]) >> bit) & 1;
                
                // Expand the expected target bit to 32 bits (0x00000000 or 0xFFFFFFFF)
                uint32_t expected_bit = (uint32_t)(-(int32_t)ks_bit);
                
                // Disable lanes that generated the wrong bit
                match_mask &= ~(z_i ^ expected_bit);
            }
            // Early reject: If all 32 keys failed this byte, exit instantly!
            if (match_mask == 0) return 0; 
        }
