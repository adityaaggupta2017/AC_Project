...
// Actual Test Vector Used From https://csrc.nist.gov/CSRC/media/Projects/lightweight-cryptography/documents/finalist-round/updated-spec-doc/grain-128aead-spec-final.pdf
static TestVector_Grain128AEADv2 grain128aeadv2_nist_tv() {
    ...
    const uint8_t nonce[12] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b};
    memcpy(tv.nonce, nonce, 12);
...
static TestVector_Grain128AEADv2 grain128aeadv2_bench_tv() {
    ...
    // Low 64 bits chosen so the correct key is the last candidate
    store_u64_le(bench_key64_last_candidate(), tv.key, 8);
...
static bool self_test_grain128aeadv2_bitsliced() {
    ...
    uint32_t match_mask = Grain128AEADv2_Bitsliced::match_keys(key_bitsliced, nonce_bitsliced, tv.ad_len, tv.pt, tv.ct, tv.pt_len);
...
static Args parse_args(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (s == "--out") a.out_csv = need("--out");
      ...
      std::cout
        << "Usage: bench [options]\n"
        << "  --cipher <simon|present|speck|grain|trivium|chacha|tinyjambu|zuc|snowv|aes|salsa|grain128aeadv2|all> (default: all)\n"
...
int main(int argc, char** argv) {
  ...
  auto run_one = [&](const std::string& name) {
    ...
    else if (name == "grain128aeadv2") benchmark_grain128aeadv2(a);
    else {
      std::cerr << "Unknown cipher: " << name << "\n";
      std::cerr << "Valid: simon, present, speck, grain, trivium, chacha, tinyjambu, zuc, snowv, aes, salsa, grain128AEADv2, all\n";
      std::exit(1);
    }
  };
